...
import { filterFiles } from "../../../filter/filterFiles";
...
async function checkDestination(
  ...
  if (
    ...
    original.isDirectory() !== target.isDirectory()
  ) {
    throw new Error(
      `Cannot replace ${destination}: a file, directory or symbolic link conflicts with the template. Move it aside and try again.`,
    );
  }
  if (original.isDirectory()) {
    ...
    throw new Error(
      `File ${destination} already exists. Use --force to replace generated files.`,
    );
  }
}
