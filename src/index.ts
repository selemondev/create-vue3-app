...
async function main() {
  ...
  if (!(
    ...
    console.error(
      `Create Vue 3 App requires Node ${packageJson.engines.node}; found ${process.versions.node}.`,
    );
    process.exitCode = 1;
    return;
  }
  program
    ...
    .description(
      "Create a Vue 3 application with Vite and your choice of tools.",
    )
    .argument(
      "[project-name]",
      "project directory (prompted when omitted in a terminal)",
    )
    ...
    .option(
      "--update-deps",
      "opt in to latest dependency majors before installing (requires npx)",
    )
    .option(
      "--install",
      "install dependencies with the invoking package manager",
    )
    .option("--no-install", "create files only; do not install dependencies")
    .addOption(
      new Option(
        "--deploy <target>",
        "prepare deployment configuration (does not deploy)",
      ).choices(["none", "vercel", "netlify"]),
    )
    .option("--git", "initialize Git and create an initial commit")
    .option("--no-git", "skip Git initialization")
    .option(
      "-y, --yes",
      "accept defaults for unanswered questions (never implies --force)",
    )
    .option(
      "--force",
      "allow replacing generated files in an existing directory",
    )
    .allowUnknownOption()
  ...
  const { default: createVueNext } =
    await import("./core/command/create-vue-next");
  await createVueNext();
}
