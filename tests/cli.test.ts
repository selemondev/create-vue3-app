...
test("legacy flags scaffold TypeScript unattended without installation", (context) => {
  ...
  const manifest = JSON.parse(
    readFileSync(join(cwd, "my-app/package.json"), "utf8"),
  );
...
test("force never writes through a nested destination symlink", (context) => {
  if (process.platform === "win32")
    return context.skip("Creating symlinks requires Windows privileges.");
  const cwd = workspace(context);
...
test("force replaces generated files but preserves unrelated files and Git", (context) => {
  ...
  assert.equal(result.status, 0, result.stdout + result.stderr);
  assert.equal(
    JSON.parse(readFileSync(join(cwd, "my-app/package.json"), "utf8")).name,
    "my-app",
  );
  assert.equal(
    readFileSync(join(cwd, "my-app/.git/keep"), "utf8"),
    "git state",
  );
  assert.equal(
    readFileSync(join(cwd, "my-app/notes.txt"), "utf8"),
    "user notes",
  );
});
...
test("rendering failure leaves an existing project unchanged", (context) => {
  ...
  const result = spawnSync(
    ...
    { cwd, encoding: "utf8", env: { ...process.env, CI: "1" }, timeout: 10000 },
  );
  assert.equal(result.status, 1);
  assert.equal(
    readFileSync(join(cwd, "my-app/package.json"), "utf8"),
  ...
  assert.equal(
    readFileSync(join(cwd, "my-app/src/App.vue"), "utf8"),
    "original app",
  );
...
test("Git initialization preserves an existing repository boundary", (context) => {
  ...
  assert.equal(
    spawnSync("git", ["commit", "-m", "existing"], { cwd, env }).status,
    0,
  );
  ...
  const result = spawnSync(
    process.execPath,
    [executable, ".", "--yes", "--no-install", "--git", "--force"],
    { cwd, encoding: "utf8", env: { ...env, CI: "1" }, timeout: 10000 },
  );
  assert.equal(result.status, 0, result.stdout + result.stderr);
  assert.equal(
    spawnSync("git", ["rev-parse", "HEAD"], { cwd, encoding: "utf8" }).stdout,
    before,
  );
});
